...
/*   Updated: 2023/12/15 16:28:07 by tmimault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (!fnl_str || (fnl_str[sizefnl] != '\n' && read_return != 0))
	{
		if (buff[fd][0] == '\0')
		{
			...
			read_return = read(fd, buff[fd], BUFFER_SIZE);
			if (read_return < 0 || (!fnl_str && read_return == 0))
				return (free(fnl_str), NULL);
		}
		fnl_str = get_next_while(buff[fd], fnl_str);
		if (fnl_str == NULL)
			return (NULL);
		sizefnl = ft_strlen(fnl_str) - 1;
	}
	return (fnl_str);
}
