...
/*   Updated: 2024/02/09 17:35:06 by tmimault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_first_b_to_a(int elem_content_b, t_list **list_a, t_list **list_b)
{
	...
	if (elem_content_b > last_elem_list(*list_a)->content)
		grand_fin++;
	if ((petit_debut == 1 && grand_fin == 1)
		|| (petit_debut == 1 && grand_fin == 0
			&& (*list_a)->first->content == min_elem_list(*list_a))
		|| (petit_debut == 0 && grand_fin == 1
			&& (last_elem_list(*list_a)->content == max_elem_list(*list_a))))
